...
COPY fastText-0.9.2.tar.gz /tmp/fasttext.tar.gz
RUN tar xzf /tmp/fasttext.tar.gz -C /tmp
RUN rm /tmp/fasttext.tar.gz
RUN make -C /tmp/fastText-0.9.2
RUN cp /tmp/fastText-0.9.2/fasttext /usr/local/bin/fasttext
